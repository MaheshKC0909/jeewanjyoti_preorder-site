...
const ECGMonitor = ({
      ...
      try {
        ...
        switch (data.type) {
          ...
          case 'ecg_request_response':
            if (data.request_id === requestIdRef.current) {
              if (data.status === 'accepted') {
                ...
                if (doctorIdRef.current && selectedPatient?.id && wsRef.current?.readyState === WebSocket.OPEN) {
                  const joinMsg = { 
                    type: 'join_live_session', 
                    doctor_id: doctorIdRef.current, 
                    user_id: selectedPatient.id 
                  };
                  console.log('Auto joining live session:', joinMsg);
          ...
          case 'ecg_data':
            ...
            if (data.heart_rate != null) setHeartRate(data.heart_rate);
            if (data.hr_variability != null) setHrVariability(data.hr_variability);
            break;
          case 'error':
  ...
  // ── ECG Waveform — renders real backend data only ─────────────────────────
  const ECGWaveform = () => {
    ...
    useEffect(() => {
      ...
      // plot real values — try common field names from backend
      const values = ecgData.map(d => d.value ?? d.amplitude ?? d.ecg_value ?? 0);
          ...
          {ecgRequestStatus === 'accepted' && (
            <div>
              ...
              <div style={s.metricsGrid}>
                ...
                <div style={s.metricCard}>
                  <div style={s.metricLabel}><Activity size={10} color={darkMode ? '#60a5fa' : '#3b82f6'} />HRV</div>
                  {hrVariability != null
                    ? <><div style={{ ...s.metricValue, color: darkMode ? '#60a5fa' : '#3b82f6' }}>{hrVariability}</div><div style={s.metricUnit}>ms SDNN</div></>
                    : <div style={s.metricEmpty}>—</div>}
                </div>
                <div style={s.metricCard}>
